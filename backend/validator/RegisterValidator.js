...
const RegisterSchema = {
    ...
    addRegisterComplete: Joi.object({
        no_rkm_medis: Joi.string()
            .pattern(/^\d{6}$/)
            .required()
            .messages({
...
const RegisterValidator = {
    ...
    validateAddRegisterNewPayload: payload => {
        const validationResult = RegisterSchema.addRegisterNew.validate(payload)
        if (validationResult.error) {
            throw new InvariantError(validationResult.error.message)
        }
    },
    ...
    validateAddRegisterCompletePayload: payload => {
        const validationResult = RegisterSchema.addRegisterComplete.validate(payload)
        if (validationResult.error) {
            throw new InvariantError(validationResult.error.message)
        }
    }
