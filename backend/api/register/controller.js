class RegisterHandler {
    ...
    async getExportExcelMonthly(req, res, next) {
        try {
            let { year, month } = req.query
            const now = new Date()
            year = year || now.getFullYear().toString()
    ...
    async getExportExcelDaily(req, res, next) {
        try {
            let { startDate, endDate } = req.query
            const now = new Date().toLocaleDateString('sv-SE')
            startDate = startDate || now
            endDate = endDate || now
            const data = await this._service.getAllRegisterByRangeDate(startDate, endDate)
    ...
    async getAllRegisterByRangeDateHandler(req, res, next) {
        try {
            let { startDate, endDate } = req.query
            const now = new Date().toLocaleDateString('sv-SE')
            startDate = startDate || now
            endDate = endDate || now
            const result = await this._service.getAllRegisterByRangeDate(startDate, endDate)
            const response = {
                error: false,
                status: 200,
                message: 'Success',
